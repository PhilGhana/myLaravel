<?php
namespace Tests\Provider\DragoonSoft;
...
class DragoonSoftBase extends TestCase
{
    ...
    {
        parent::setUp();
        $this->platform = GamePlatform::where('key', 'DragoonSoft')->first();
    }

    public function requestData(MemberPlatformActive $active)
    {
        return [
            'agent' => 'IS880002UAT',
            'account' => $active->player_id,
        ];
    }
    ...
    {
        /** @var Member $member */
        $member = Member::where('account', 'ccmem01')->first();
        return $member;
    }
