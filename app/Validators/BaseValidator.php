...
class BaseValidator
{
    # 自訂義檢查規則 (參考 https://laravel.tw/docs/5.2/validation#available-validation-rules)
    protected $rules = [];

    # 自訂義錯誤說明 (參考 https://laravel.tw/docs/5.2/validation#working-with-error-messages)
    protected $messages = [];
    ...
    {
        if ($this->fails()) {
            throw (new FailException())->setErrors($this->errors());
