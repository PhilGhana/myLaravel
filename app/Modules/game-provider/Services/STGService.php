...
class STGService extends BaseWalletService
{
    ...
    public function cancelOrder()
    {
        ...
        return DB::transaction(function () use ($logWallet, $rlog, $lastrequstid) {
            ...
            $this->logTransaction($this->logProvider->report_id, $lastrequstid, $rlog);
    ...
    public function doSettleNotWin($gid, $cal_bet, $settleAt = null, $gameAt = null, $teamc_score = null, $teamh_score = null)
    {
        ...
        // 因為只回傳贏的，所以其他輸都改成注單完成
        $sqlReport->update(['status' => Report::STATUS_COMPLETED, 'settle_at' => $settleAt]);
    }
    ...
    public function doSyncReportsSTG($rows, SyncReportParameter $srp, $requestid, $cal_bet = true)
    {
        ...
        foreach ($rows as $row) {
            ...
            try {
                ...
                // 阻擋同一筆對獎重複跑
                $cnt = LogProviderTransaction::on('write_log')->where('transaction_id', $requestid)->where('report_id', $report->id)->count();
