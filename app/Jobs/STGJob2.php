...
//         // 如果碰到取消賽事訊號
//         if($this->cr == '-1' && $this->hr == '-1')
//         {
//             $STGservice->cancelSettle($this->gid, $this->requestid);
//             return;
//         }

//         // 碰到待定
//         if($this->cr == '-2' && $this->hr == '-2')
//         {
//             $STGservice->waitSettle($this->gid, $this->requestid);
//             return;
//         }
...
//         // 一定要把request id換掉, 不然會被擋下來
//         $rid = $requestid . '-resettle';

//         $STGservice = new STGService(json_decode($this->platform->setting, true));

//         // 撈出所有變更單號 鎖定 lockForUpdate
//         $logProviders = LogProviderTransaction::with(['log'])->where('transaction_id', $requestid)
//                         ->where('platform_id', $this->platform->id)
//                         ->lockForUpdate()
//                         ->get();
...
//                     // 扣錢
//                     // 收不到錢會跳走
//                     $STGservice->logTransaction($logProvider->report_id, $rid, $rlog);
//                 }
...
//                 // 重新派彩要清空退庸
//                 ReportCommission::where('id', $report->id)->delete();
